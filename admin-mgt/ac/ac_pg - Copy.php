...
if (isset($_GET['qpr'])) {
	...
	if($get_action_id=="regs"){
		...
		if ($getRef['status']==1) {
			...
			//check password
			if ($getRef['data']['ref_password']==$password) {
				//session_destroy();
				//session_start();
				if ($getRef['data']['status']=='Uploaded') {
					
					$response['status_msg']=$getRef['status_msg']." <BR><BR> PS: Reference has already been uploaded by this user. An update will replace current file in the system";
				}else{
					$response['status_msg']=$getRef['status_msg'];
				}
				
				$response['status']=$getRef['status'];
				$_SESSION['REFID']=$refID;
				$_SESSION['pa_id']=$getRef['data']['User_ID'];
			}else{
				$response['status']=0;
				$response['status_msg']="Sorry password incorrect. Please copy password from email and try again. If problem persists contact <a href='mailto:pymireku@aau.org' target='_blank'>Technical Support</a>".$refID;
			}
		}else{
			$response=$getRef;
		}
